...
speclagreg = function(X,Y,Kconst=1,K=NULL,lags=0:0,freq=NULL,p=10,q=10,weights="Bartlett",method=0)
{
...
#    K = speclagreg.K(X,Y,freq=thetas,lags=lags,p=p,q=q,weights=weights)
    K = speclagreg.K.experimental(X,Y,freq=thetas,method=method,SXX=SXX)
  }
